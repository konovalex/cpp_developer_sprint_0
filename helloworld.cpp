...
void AddDocument(vector<pair<int, vector<string>>>& documents, const set<string>& stop_words,
    ...
    pair<int, vector<string>> DocumentWordsPair;
    DocumentWordsPair.first = document_id;
    DocumentWordsPair.second = words;
    documents.push_back(DocumentWordsPair);
}
...
int MatchDocument(const pair<int, vector<string>>& content, 
    ...
    int FindWordInDocument = 0;
    vector<string> word = content.second;
    for (string w : content.second) {
      if (query_words.count(w) > 0) {
        ++FindWordInDocument;
      }
    }
    return FindWordInDocument;
}
